...
class VoiceLevel(commands.Cog):
    ...
    async def join(self, user, before, after):
        if before.channel is None and after.channel is not None:
            ...
            voicetime = self.bot.dbcursor.execute(
                f"SELECT voicetime FROM levels WHERE id = {user.id}"
            ).fetchone()[0]
            ...
            if voicetime > 600:
                self.bot.dbcursor.execute(
                    f"UPDATE levels SET level = level + 1 WHERE id = {user.id}"
                )
                self.bot.dbcursor.execute(
                    f"UPDATE levels SET voicetime = 0 WHERE id = {user.id}"
                )

                voicelevel = self.bot.dbcursor.execute(
                    f"SELECT voicelevel FROM levels WHERE id = {user.id}"
                ).fetchone()[0]
                embed = Embed(
